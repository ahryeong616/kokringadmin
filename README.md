...
같은 서버·같은 저장소를 쓰므로 재고관리와 함께 한 번에 배포됩니다.

- 주소: `배포주소/survey`
- 기록은 기기(IndexedDB)에 먼저 저장되어 **신호가 없는 창고에서도** 조사가 끊기지 않습니다.
- 내보내기 탭에서 **동기화 켜기** + `APP_ACCESS_KEY` 를 넣으면 폰·PC가 같은 기록을 봅니다.
- 사진·동영상은 용량 때문에 기기에만 남습니다. 글자 정보만 서버로 맞춥니다.

배포 방법은 `시세조사_배포방법.md` 를 보세요.
